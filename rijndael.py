...
#
#18 July 2018 - add CBC mode by Jason Reaves
...
class Rijndael(object):
    ...
    def decrypt_cbc(self, ciphertext, iv=None):
        if iv == None:
            iv = '\x00'*self.block_size
        if len(iv) != self.block_size:
            raise ValueError('wrong length of IV')
        ...
        for i in range((len(ciphertext)/self.block_size)):
            temp = self.decrypt(ciphertext[i*self.block_size:(i+1)*self.block_size])
            to_decode = ""
            for j in range(4):
                temp1 = struct.unpack_from('<I', temp[j*4:])[0]
                temp2 = struct.unpack_from('<I', last[j*4:])[0]
                to_decode += struct.pack('<I', ((temp1 ^ temp2) & 0xffffffff))
            last = ciphertext[i*self.block_size:(i+1)*self.block_size]
            out += to_decode
        return out
    # @staticmethod
